...
def main():
    ...
    mouthroi_path = opt.testset_root + "/mouth_roi_hdf5/" + opt.testset_type + "/"
    audio_path = opt.testset_root + "/aac/" + opt.testset_type + "/"
    video_path = opt.testset_root + "/mp4/" + opt.testset_type + "/"

    test_ids = []
    for i in range(opt.test_count):
        ids = random.sample(os.listdir(video_path), 2)
        iids = [id + "/" + random.choice(os.listdir(video_path + "/" + id)) for id in ids]
        test_ids.append([id + "/" + random.choice(os.listdir(video_path + "/" + id)) for id in iids])
        test_ids[-1] = [id[:-4] for id in test_ids[-1]]

    if os.path.isdir(opt.output_dir_root):
        shutil.rmtree(opt.output_dir_root)
    os.mkdir(opt.output_dir_root)

    for id in test_ids :
        # load data	
        mouthroi_1 = load_mouthroi(mouthroi_path + id[0] + ".h5")
        mouthroi_2 = load_mouthroi(mouthroi_path + id[1] + ".h5")

        _, audio1 = wavfile.read(audio_path + id[0] + ".wav")
        _, audio2 = wavfile.read(audio_path + id[1] + ".wav")
        audio1 = audio1 / 32768
        audio2 = audio2 / 32768
        ...
        v1 = video_path + id[0] + ".mp4"
        v2 = video_path + id[1] + ".mp4"

        if opt.reliable_face:
            best_score_1 = 0
            best_score_2 = 0
            for i in range(10):
                frame_1 = load_frame(v1)
                frame_2 = load_frame(v2)
