    ...
    def __getitem__(self, index):
        ...
        for i in range(self.opt.number_of_identity_frames):
            ...
            for j in range(num_frames):
                ...
                frame_Av.append(self.vision_transform(frame_A))
                frame_Bv.append(self.vision_transform(frame_B))
                # frame_B = self.vision_transform(frame_B)
            frame_A1 = (torch.stack(frame_Av, axis = 0)).mean(axis=0)
            frame_B1 = (torch.stack(frame_Bv, axis = 0)).mean(axis=0)
            frame_A_list.append(frame_A1)
            frame_B_list.append(frame_B1)
