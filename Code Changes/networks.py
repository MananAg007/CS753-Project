...
class VisualVoiceUNet(nn.Module):
    def __init__(self, ngf=64, input_nc=2, output_nc=2, audioVisual_feature_dim=1152):
        ...
        self.Tanh = nn.Tanh()
        self.dim = audioVisual_feature_dim
    ...
    def forward(self, audio_mix_spec, visual_feat, activation='Sigmoid'):
        ...
        print("A",audioVisual_feature.shape)
        queries = self.query(audioVisual_feature)
        keys = self.key(audioVisual_feature)
        values = self.value(audioVisual_feature)
        scores = torch.bmm(queries, keys.transpose(1,2)) / self.dim ** 0.5
        attention = self.softmax(scores)
        weighted = torch.bmm(attention, values)
        print("W", weighted.shape)
        audio_upconv1feature = self.audionet_upconvlayer1(weighted)
